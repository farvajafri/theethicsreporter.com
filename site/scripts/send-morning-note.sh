...
content = open('/Users/farvascott/code/theethicsreporter/site/.env.local').read()
...
key = open('/Users/farvascott/code/theethicsreporter/site/.env.local').read().split('BREVO_API_KEY=\"')[1].split('\"')[0].replace('\\\\n','').replace('\\n','').strip()
...
while IFS='|' read -r EMAIL FIRSTNAME; do
    ...
    HTML=$(python3 -c "
import sys
html = open('/Users/farvascott/code/theethicsreporter/site/scripts/morning-note.html').read()
